#ifndef PARASITIC_C_FILE__3ED
#define PARASITIC_C_FILE__3ED
...
// FN DECLS //
struct Lexer lexer_init(char *filename, char *input);
bool lexer_starts_with(struct Lexer *lxr, char *target);
bool lexer_accept_space(struct Lexer *lxr);
bool lexer_allow_whitespace(struct Lexer *lxr);
bool lexer_accept_ident(struct Lexer *lxr);
bool lexer_accept_symbol(struct Lexer *lxr, char *expected_symbol, enum TokTag expected_tok_tag);
bool lexer_accept_keyword(struct Lexer *lxr, char *expected_kw, enum TokTag expected_tok_tag);
bool lexer_accept_literal_int(struct Lexer *lxr);
bool lexer_accept_literal_char(struct Lexer *lxr);
bool lexer_accept_literal_string(struct Lexer *lxr);
bool lexer_accept_angle_bracket_filename(struct Lexer *lxr);
bool lex(struct Lexer *lxr);
...
int dbg_tok_tag(FILE *out, enum TokTag tok_tag);

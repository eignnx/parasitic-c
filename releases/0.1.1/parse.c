...
tags_enum->tag = TYPE_ANON_ENUM;
tags_enum->as.anon_enum.enum_members = tags;
{
...
anon_struct->as.anon_struct_or_union.struct_members = dm->struct_members;
struct StructMember *sm = malloc(sizeof (*sm));
sm->field = mangled_name;
sm->type = anon_struct;
...
union_of_structs->tag = TYPE_ANON_UNION;
union_of_structs->as.anon_struct_or_union.struct_members = struct_members;
...
char *variant_name = lxr->token;
lexer_expect(lxr, TOK_OPEN_BRACE);
struct List init_list = parse_struct_initializer_list(lxr);
lexer_expect(lxr, TOK_CLOSE_BRACE);
expr = malloc(sizeof (*expr));

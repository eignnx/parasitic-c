#ifndef PARASITIC_C_FILE__3E8
#define PARASITIC_C_FILE__3E8
...
void test();
void usage();
void comp_log(char *msg);
char * remove_parac_extension(char *filename);
bool endswith(char *str, char *suffix);
bool is_parac_file(char *filename);
struct List all_included_parac_filenames(struct TranslationUnit *tu);
char * dirname(char *path);
char * basename(char *path);
char * path_join(char *p1, char *p2);
struct List compile_file(char *filename, char *dir_root);
void compile_project(struct Compiler *compiler, char *project_root);

#ifndef PARASITIC_C_FILE__0000018DC390F390
#define PARASITIC_C_FILE__0000018DC390F390
#include <stdio.h>
#include <stdlib.h>
#include "lex.c"
#include "list.c"
...
fndecl(bool display_param_types(FILE*, struct List*));
fndecl(bool display_type(FILE*, struct Type*));
fn(bool display_fn_pointer_type(FILE* out, struct Type* type, char* binder))
{
...
fndecl(bool display_fn_params(FILE*, struct List*));
fn(bool display_fn_signature(FILE* out, char* name, struct List* fn_params, struct Type* ret_type))
{
...
fn(bool display_typed_identifier(FILE* out, struct Type* type, char* ident))
{
char* space = " ";
...
fndecl(struct List parse_param_types(struct Lexer*));
...
case ITEM_FN_DECL:
{
return display_fn_signature(out, item->as.fn_decl.name, &item->as.fn_decl.param_types, item->as.fn_decl.ret_type) && fprintf_s(out, ";\n\n") >= 0;
}
case ITEM_FN_DEF:
{
...
global(int NEXT_TU_ID) = 1000;
struct TranslationUnit {
    int id;
    struct List directives;
    struct List type_defns;
    struct List global_defns;
    struct List fn_defns;
};
fn(struct TranslationUnit translation_unit_init())
{
struct TranslationUnit tu;
tu.id = NEXT_TU_ID;
NEXT_TU_ID = NEXT_TU_ID + 1;
tu.directives = list_init();
tu.type_defns = list_init();
tu.global_defns = list_init();
tu.fn_defns = list_init();
return tu;
}
fn(void translation_unit_push_item(struct TranslationUnit* tu, struct Item* item))
{
switch (item->tag)
{
case ITEM_POUND_INCLUDE:
...
case ITEM_FN_DECL:
{
fprintf_s(stderr, "[INFO] fn declarations are no longer needed.\n");
return;
}
}
}
fndecl(struct Item* parse_item(struct Lexer*));
fn(struct TranslationUnit parse_translation_unit(struct Lexer* lxr))
{
...
fn(bool display_item_list(FILE* out, struct List* items))
{
...
fn(bool display_tu_type_decls(FILE* out, struct TranslationUnit* tu))
{
struct ListNode* node = tu->type_defns.first;
while (node)
{
struct Item* item = (struct Item*) node->data;
switch (item->tag)
...
fn(bool display_tu_fn_decls(FILE* out, struct TranslationUnit* tu))
{
...
fn(void test_typed_ident(char* input))
{
struct Lexer lxr;
struct Type* type;
printf("\n");
